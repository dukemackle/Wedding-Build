  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
      {entries.map((entry, i) => (
        <FadeInSection
          key={entry.id}
          delayMs={Math.min(i * 50, MAX_STAGGER_MS)}
          className={entry.photo_url ? undefined : "col-span-2 sm:col-span-3"}
        >
          {entry.photo_url ? (
            <div className="overflow-hidden rounded-lg border border-hairline bg-parchment shadow-sm">
              <Image
                src={entry.photo_url}
                alt={entry.name}
                width={300}
            ...
            <div className="flex gap-3 rounded-md border border-hairline bg-parchment p-4">
